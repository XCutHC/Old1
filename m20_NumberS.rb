...
nr = str.scan(/\d/).map(&:to_i).inject(:+)
let = str.scan(/[a-zA-Z]/).size
ans =(nr / let.to_f).round
puts ans
end
...
# "Hello6 9World 2, Nic8e D7ay!" the output should be 2. First if you add up all the
# numbers, 6 + 9 + 2 + 8 + 7 you get 32. Then there are 17 letters in the string.
# 32 / 17 = 1.882, and the final answer should be rounded to the nearest whole number,
# so the answer is 2. Only single digit numbers separated by spaces will be used
